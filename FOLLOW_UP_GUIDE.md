...
### Liquid-glass legibility fix — frost restored in the refract chain + night-glass recipe (2026-07-22)
- Request: three glass surfaces looked unnatural — the HomePrograms strengths modal (backdrop text bled through the panel and mixed with the modal copy), the footer 예약 안내/찾아 오시는 길 pills on the dark zone, and the HomeNavigationGallery quick-route chips (VIP/VVIP/PRESTIGE) over photography.
- Diagnosis (all in `globals.css`; no component changes needed):
  - Every `.glass*` class declared `-webkit-backdrop-filter: blur(…) saturate(…)` followed by `backdrop-filter: var(--glass-refract)` = `url(#liquidGlass) saturate(…)`. In Chromium `-webkit-backdrop-filter` is an alias of `backdrop-filter`, so the url() chain OVERRIDES the blur declaration — and the `#liquidGlass` SVG filter (layout.tsx) only displaces (scale 54) with a 1.2px internal blur → glass rendered with effectively NO frost, so the backdrop bled through every glass surface. (Safari/Firefox parse-reject url() inside backdrop-filter and fall back to the -webkit blur, as the original comment intended — the bug was Chromium-specific.)
  - `.glass-on-dark` reused the light-glass gloss (`--glass-sheen-strong` with a 0.72-alpha white streak + the bright `--glass-specular`), which reads as a milky gray smear on dark/photo backdrops — the "dirty pill" look.
- Change (`globals.css` only):
  - `--glass-refract`/`--glass-refract-heavy` now carry the frost inside the chain: `blur(var(--glass-blur[-heavy])) url(#liquidGlass) saturate(…)`.
  - Each glass class also declares a plain unprefixed `backdrop-filter: blur+saturate` line before the refract line (fallback for engines that parse-reject url()).
  - `--glass-fill-prominent` 0.2 → 0.55 — prominent chrome hosts reading surfaces (HomePrograms + Location modals, FloatingReservationCta, AIAgentFAB, HomeExperienceShowcase stat card), which need legible copy over busy backdrops.
  - New `--glass-sheen-dark` + `--glass-specular-dark`; `.glass-on-dark` retuned: fill 0.16→0.12, border white/0.5→0.45, dark sheen/specular. Applies consistently to the footer pills, gallery chips/chevron, spa & about dark chips, `GlassChip onDark`/`GlassCard tone="dark"`.
  - `.glass-bar` switched to plain `blur(24px) saturate(1.9)` for the unprefixed line — its own comment says nav chrome should be clean frost with no displacement wobble, but it was on the refract chain (so the header also had no frost in Chromium).
  - Deleted 4 untracked byte-identical Finder duplicates (`GlassButton 2.tsx`, `GlassCard 2.tsx`, `GlassChip 2.tsx`, `GlassIconButton 2.tsx`) in `ui/glass/` — verified `cmp` identical and unimported.
- Validation: `pnpm build` passes. Visual verification in the preview pane (Chromium): strengths modal now clean over the busy grid (no ghost text), footer pills read as quiet night glass, VIP/VVIP/PRESTIGE chips sit naturally on the wood photo; computed style on a chip confirms `blur(16px) url("#liquidGlass") saturate(1.8)` wins the cascade in Chromium.
- Pane caveats hit during verification (recurring; see also earlier "screenshots unreliable" notes): the SVG displacement chain is heavy enough in the pane's software rasterizer that the compositor freezes on glass-dense sections — screenshots were taken with a temporarily injected override forcing the plain-blur fallback (i.e. the Safari path), plus one real-chain screenshot at the hero (glass-prominent FAB frosted correctly). Also `computer scroll` actions hang (Lenis never "settles"); drive scrolling by dispatching synthetic `WheelEvent`s from JS with an asymptotic stop condition instead.
- Perf note for a future pass: `url(#liquidGlass)` displacement runs on many large surfaces and feTurbulence/feDisplacementMap rasterize on CPU in Chromium — low-end mobile may struggle. If perf reports come in, the cheap lever is dropping `url(#liquidGlass)` from the refract vars: the plain frost fallback already looks correct everywhere.
